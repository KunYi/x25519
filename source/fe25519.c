...
void fe25519_invert(fe25519_t* out, const fe25519_t* z)
{
  ...
  /* Based on fe_invert from ref10 with slight modifications to avoid unnecessary copies. */
  /* qhasm: fe z1 */
  /* qhasm: fe z2 */
  /* qhasm: fe z8 */
  /* qhasm: fe z9 */
  ...
  fe25519_square(&t1, &t0);	     /* t1 = z^(2^2) */
  for (int i = 1;i < 2; ++i) {
    fe25519_square(&t1, &t1);        /* t1 = z^(2^2^2) */
  }
  ...
  fe25519_square(&t2, &t0);
  for (int i = 1;i < 1; ++i) {
    fe25519_square(&t2, &t2);
  }
